...
export function AssessmentCard({ assessment, onClick }: AssessmentCardProps) {
    return (
        <div
            className="bg-white border border-[var(--color-gray-200)] rounded-xl p-[var(--space-4)] cursor-pointer hover:shadow-md transition-shadow"
            onClick={onClick}
        >
            {/* Header */}
            ...
            {/* Footer */}
            <div className="flex items-center justify-between pt-[var(--space-3)] border-t border-[var(--color-gray-100)]" onClick={(e) => e.stopPropagation()}>
                <Button
                    variant="outline"
                    size="sm"
                    className="gap-[var(--space-2)] text-[13px] h-8 border-[var(--color-gray-300)]"
                >
